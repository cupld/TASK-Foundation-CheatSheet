...
`ls -ltr`: is used to list all items in a folder
...
`git add filename`: is the first step to be able to commit and push a git repository. We should add all file in the working directory to **staging** status by using the `___` notation
